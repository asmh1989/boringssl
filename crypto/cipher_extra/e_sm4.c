...
static int SM4_set_key(const uint8_t *userKey, size_t length, SM4_KEY *key)
{
    uint32_t *rk = key->key;
    uint32_t K[5];
...
static void SM4_encrypt(const uint8_t *in, uint8_t *out, const SM4_KEY *key)
{
...
static void SM4_decrypt(const uint8_t *in, uint8_t *out, const SM4_KEY *key)
{
    const uint32_t *rk = key->key;
    uint32_t X[5];
...
static void SM4_cbc_encrypt(const uint8_t *in, uint8_t *out, size_t length, const SM4_KEY *key, uint8_t *ivec, const int enc)
{
    if (enc)
...
static int sm4_init_key(EVP_CIPHER_CTX *ctx, const uint8_t *key, const uint8_t *iv, int enc)
{
...
static int sm4_cbc_cipher(EVP_CIPHER_CTX *ctx, uint8_t *out, const uint8_t *in, size_t inl)
{
